...
class Button:
    def __init__(self, normal_image_path, x, y, action=None, fallback_size=(200, 60)):
        ...
        self.image_normal = self.load_button_image(
            normal_image_path, fallback_size, (70, 130, 180)
        )
        hover_image_path = normal_image_path.replace("_normal", "_hover")
        self.image_hover = self.load_button_image(
            hover_image_path, fallback_size, (100, 180, 220)
        )
        pressed_image_path = normal_image_path.replace("_normal", "_pressed")
        self.image_pressed = self.load_button_image(
            pressed_image_path, fallback_size, (100, 180, 220)
        )
...
class Message:
    def __init__(
        ...
        padding=20,
        height=40,
        center_image_path="message_center.png",
        font_size=20,
        font_name=None,
    ):
        self.text = text
        ...
        self.img_center = self.load_image(
            center_image_path, (20, self.height), fallback_color
        )
    ...
    def load_image(self, path, fallback_size, fallback_color):
        if os.path.exists(path):
            ...
            return pygame.transform.scale(
                original_image, (original_image.get_width() * 2, self.height)
            ).convert_alpha()
        else:
    ...
    def render_colored_text(self, text):
        ...
        combined_surf = pygame.Surface(
            (max(1, total_width), max(1, max_height)), pygame.SRCALPHA
        )
    ...
    def animate(self):
        ...
        self.current_width = (
            self.start_width + (self.target_width - self.start_width) * cubic_out
        )
    ...
    def update_visuals(self):
        ...
        self.rect = self.surface.get_rect(
            midtop=(self.screen_width // 2, int(self.current_y))
        )
        ...
        if width >= lw + rw:
            ...
            scaled_center = pygame.transform.scale(
                self.img_center, (center_width, self.height)
            )
...
class MainMenuUI:
    def __init__(self, on_play, on_settings, on_quit):
        ...
        self.play_button = Button(
            "assets/images/gui/play_normal.png", x=320, y=150, action=on_play
        )
        ...
        self.quit_button = Button(
            "assets/images/gui/quit_normal.png", x=320, y=225, action=on_quit
        )
...
class WirePuzzleUI:
    def __init__(self, audio_system, num_wires=4, on_solve=None, on_close=None):
        ...
        try:
            self.left_metal_img = pygame.image.load(
                "assets/images/gui/metal_left.png"
            ).convert_alpha()
            self.left_metal_img = pygame.transform.scale2x(self.left_metal_img)

            self.right_metal_img = pygame.image.load(
                "assets/images/gui/metal_right.png"
            ).convert_alpha()
            self.right_metal_img = pygame.transform.scale2x(self.right_metal_img)

            self.background = pygame.image.load(
                "assets/images/gui/wires_puzzle_background.png"
            ).convert_alpha()
    ...
    def _init_nodes(self):
        ...
        step = (
            available_height // max(1, (self.num_wires - 1))
            if self.num_wires > 1
            else 0
        )
    ...
    def handle_event(self, event):
            ...
            if self.active_wire:
                ...
                for right_node in self.right_nodes:
                    if self._is_near(mouse_pos, right_node["pos"]):
                        ...
                        self._check_victory()
                        self.audio_system.play_sfx(
                            f"connect_light_{random.choice((1, 2, 3, 4))}"
                        )
                        break
                self.active_wire = None
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == 3:
            ...
            for right_node in self.right_nodes:
                if (
                    self._is_near(mouse_pos, right_node["pos"])
                    and right_node["connected"]
                ):
                    ...
                    for left_node in self.left_nodes:
                        if left_node["target_pos"] == right_node["pos"]:
                            ...
                            self.audio_system.play_sfx(
                                f"electric_shock_{random.choice((1, 2, 3))}"
                            )
                            break
                    self._check_victory()
                    break
    ...
    def draw_wire(self, screen, start, target, color, shading_count=10):
        ...
        for i in range(shading_count):
            pygame.draw.line(
                screen,
                (
                    int(r * (shading_count - i) * step),
                    int(g * (shading_count - i) * step),
                    int(b * (shading_count - i) * step),
                ),
    ...
    def draw(self, screen):
        ...
        for node in self.left_nodes + self.right_nodes:
            ...
            pygame.draw.circle(
                screen, (20, 22, 26), node["pos"], self.slot_size[0] // 2
            )
            pygame.draw.circle(
                screen, (70, 75, 85), node["pos"], self.slot_size[0] // 2, width=1
            )
        ...
        if self.active_wire:
            self.draw_wire(
                screen,
                self.active_wire["pos"],
                self.mouse_pos,
                self.active_wire["color"],
            )
        ...
        if self.left_metal_img and self.right_metal_img:
            ...
            screen.blit(
                self.right_metal_img,
                self.right_metal_img.get_rect(topright=(screen.get_width(), 0)),
            )
        else:
            pygame.draw.rect(
                screen,
                (45, 48, 54),
                (*self.left_panel_pos, self.panel_width, self.panel_height),
                border_radius=4,
            )
            pygame.draw.rect(
                screen,
                (45, 48, 54),
                (*self.right_panel_pos, self.panel_width, self.panel_height),
                border_radius=4,
            )
...
class VaultPuzzleUI:
    def __init__(self, audio_system, solution=None, on_solve=None, on_close=None):
        ...
        self.background = pygame.transform.scale2x(
            pygame.image.load(
                "assets/images/gui/vault_puzzle_background.png"
            ).convert_alpha()
        )
    ...
    def _on_button_click(self, label):
        ...
        if label == "insert":
            ...
            if len(self.current_input) == len(self.solution) and all(
                [
                    self.current_input[i] == self.solution[i]
    ...
    def draw(self, screen):
        ...
        if self.error_timer > 0:
            ...
            pygame.draw.rect(
                screen, (80, 85, 95), self.screen_rect, width=2, border_radius=8
            )
        ...
        for i in range(4):
            if i < len(self.current_input):
                display_slots.append(
                    ("-", (0, 255, 150))
                )  # Bright cyan/green for entered code
            else:
...
class StoragePuzzleUI:
    def __init__(self, audio_system, solution=None, on_solve=None, on_close=None):
        ...
        self.background = pygame.transform.scale2x(
            pygame.image.load(
                "assets/images/gui/storage_puzzle_background.png"
            ).convert_alpha()
        )
...
class AiChatUI:
    ...
    def draw(self, screen):
        ...
        history_surface = pygame.Surface(
            (self.history_rect.width, self.history_rect.height), pygame.SRCALPHA
        )
        history_surface.fill((10, 14, 20, 210))
        screen.blit(history_surface, self.history_rect.topleft)
        pygame.draw.rect(
            screen, (60, 70, 85), self.history_rect, width=2, border_radius=8
        )
        ...
        for idx, line_data in enumerate(lines_to_draw):
            text_surf = self.chat_font.render(
                line_data["text"], True, line_data["color"]
            )
            screen.blit(
                text_surf, (self.history_rect.left + 15, start_y + (idx * line_height))
            )

        # DRAW BOTTOM INPUT TRAY
        tray_rect = pygame.Rect(0, screen.get_height() - 70, screen.get_width(), 70)
        tray_surface = pygame.Surface(
            (tray_rect.width, tray_rect.height), pygame.SRCALPHA
        )
